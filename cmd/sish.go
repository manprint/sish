...
func init() {
	...
	rootCmd.PersistentFlags().BoolP("auth-users-enabled", "", false, "Enable SSH password authentication using per-user credentials loaded from YAML files")
	rootCmd.PersistentFlags().StringP("auth-users-directory", "", "", "Directory containing YAML files with SSH auth users and passwords.\nsish will watch this directory and automatically reload users on file changes")
	...
	rootCmd.PersistentFlags().StringP("forwarders-log", "", "disable", "Enable or disable per-forwarder logs. Allowed values: enable|disable")
	rootCmd.PersistentFlags().StringP("forwarders-log-dir", "", "/fwlogs", "Directory where per-forwarder log files are written")
	rootCmd.PersistentFlags().StringP("forwarders-log-time-format", "", "", "Time format for per-forwarder log lines. Defaults to --time-format when empty")
	...
	rootCmd.PersistentFlags().BoolP("ssh-over-https", "", false, "Allow SSH tunnel connections over the HTTPS listener port by multiplexing SSH and TLS traffic")
	...
	rootCmd.PersistentFlags().BoolP("forwarders-log-compress", "", false, "Enable compressing rotated per-forwarder log files")
	...
	rootCmd.PersistentFlags().BoolP("bandwidth-hot-reload-enabled", "", false, "Enable periodic hot-reload of per-user bandwidth limits for active SSH connections")
	...
	rootCmd.PersistentFlags().IntP("forwarders-log-max-size", "", 100, "Maximum size of each per-forwarder log file in megabytes before rotation")
	rootCmd.PersistentFlags().IntP("forwarders-log-max-backups", "", 10, "Maximum number of rotated per-forwarder log files to keep")
	rootCmd.PersistentFlags().IntP("forwarders-log-max-age", "", 30, "Maximum number of days to keep rotated per-forwarder log files")
	...
	rootCmd.PersistentFlags().DurationP("auth-users-directory-watch-interval", "", 200*time.Millisecond, "The interval to poll for filesystem changes for SSH auth users")
	rootCmd.PersistentFlags().DurationP("bandwidth-hot-reload-time", "", 20*time.Second, "How often to apply refreshed per-user bandwidth limits to active SSH connections")
	rootCmd.PersistentFlags().DurationP("headers-setting-directory-watch-interval", "", 200*time.Millisecond, "The interval to poll for filesystem changes for headers settings")
	rootCmd.PersistentFlags().DurationP("census-refresh-time", "", 2*time.Minute, "How often to refresh census data from census-url")
